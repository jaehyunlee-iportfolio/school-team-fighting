...
- **Firebase App Check** 적용: Cloud Firestore와 Authentication에 App Check가 활성화되어 있어, 승인된 앱에서 보낸 요청만 Firebase 백엔드에 도달합니다.
- 모든 Firebase 호출은 클라이언트 SDK로 이루어집니다. Next.js 미들웨어나 서버 액션은 사용하지 않으므로, Firestore Security Rules로 서버 측 접근 제어를 보완합니다.
...
| CSV 파싱 | PapaParse |
| 파일 압축 | JSZip |
| 인증/DB | Firebase Auth (Google), Firestore, App Check |
| 토스트 | Sonner |
| 아이콘 | Lucide React |
| 배포 | Vercel |
...
- Node.js 18 이상
- Firebase 프로젝트 (Authentication + Firestore + App Check)
...
4. **App Check** → Cloud Firestore, Authentication에 적용 (reCAPTCHA Enterprise 또는 reCAPTCHA v3 등 attestation provider 연결)
5. Firestore 보안 규칙을 설정하여 `admin_emails`와 `settings` 컬렉션의 쓰기를 어드민 사용자로 제한
