...
| PDF 레이아웃 커스터마이징 | 폰트, 크기, 여백, 테이블 치수, 색상, 누락 데이터 표시 등 40+ 항목을 어드민에서 조정 가능. 목 데이터 기반 실시간 PDF 미리보기 제공 |
...
- **미리보기 (preview):** 2단계에서 행을 선택하면 PDF를 iframe으로 미리보기. "ZIP으로 받기" 클릭 시 전체 PDF를 ZIP 한 개로 다운로드
...
| 탭 | 내용 |
|----|------|
| 서명 정책 | 그룹별(iPF/디미교연) 결재자 서명 이미지 업로드/교체/삭제. 업로드 시 흰 배경 자동 투명화 + 민감도 슬라이더 + 수동 크롭 다이얼로그 |
| 결재 그룹 | iPF/디미교연 각 그룹의 결재자 1, 2 직위 라벨 편집 |
| PDF 레이아웃 | PDF 출장신청서의 폰트·크기·여백·테이블 치수·색상 등 40+ 항목 편집. 좌측에 목 데이터 기반 실시간 PDF 미리보기 패널 제공. 기본값 초기화 가능 |
...
└── lib/
    ...
    ├── image/
    │   └── remove-bg.ts          # 서명 이미지 흰 배경 투명화, 자동 크롭 영역 감지, 크롭 실행
    ├── firebase/
